...
class Vocabulary:
    def __init__(self, vocab_path: str | Path) -> None:
        ...
        if isinstance(raw, dict) and isinstance(raw.get("model"), dict) and "vocab" in raw["model"]:
            token_to_id: dict[str, int] = raw["model"]["vocab"]
        else:
            token_to_id = raw

        self._id_to_token: dict[int, str] = {v: k for k, v in token_to_id.items()}

        all_ids = set(self._id_to_token)

        self.structural: frozenset[int] = frozenset(
            i for i in all_ids if self.surface(i) in {"{", "}", '"', ":", ",", "[", "]"}
        )
        self.digit_like: frozenset[int] = frozenset(
            i for i in all_ids if re.fullmatch(r"[0-9.+\-eE]+", self.surface(i))
        )
        self.string_safe: frozenset[int] = frozenset(
            i
            for i in all_ids
            if '"' not in self.surface(i) and not _CONTROL_CHAR_RE.search(self.surface(i))
        )
